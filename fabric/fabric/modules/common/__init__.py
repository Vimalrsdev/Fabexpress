...
def validate_discount_code(discount_code, source, EGRN, CustomerCode, BranchCode, is_er_discount):
    """
    ...
    if source == "order":
        ...
        query = f"EXEC {SERVER_DB}..validateDiscountCode @PROMOCODE='{discount_code}',@source='{source}',@CUSTOMERCODE='{CustomerCode}',@Branchcode='{BranchCode}',@EGRNNo='{EGRN}',@EasyDiscount='{is_er_discount}'"
    else:
        # source can also be pickup.
        query = f"EXEC {SERVER_DB}..validateDiscountCode @PROMOCODE='{discount_code}',@source='{source}',@CUSTOMERCODE='{CustomerCode}',@Branchcode='{BranchCode}',@EasyDiscount='{is_er_discount}'"
...
def send_out_for_activity_sms(alert_code, CustomerName, MobileNumber, booking_id, egrn):
    """
...
def send_reschedule_sms(alert_code, CustomerName, MobileNo, EGRN, garments_count, RescheduleDate):
    """
